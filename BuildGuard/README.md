...
対象は現在のBuild Profileで有効なシーンにある、最も外側で接続中のプレハブ実体です。ゲームオブジェクトとコンポーネントの追加・削除だけを扱い、`Transform`を含むプロパティ値の変更は表示しません。結果はパス順へ安定させた検査時点の一覧で、最大1,000件を表示します。中止、または1件でもシーン検査に失敗した場合は途中結果を全て破棄します。
