...
namespace ModuleInstaller.Editor.Tests
{
    internal sealed class ModuleCatalogTests
    {
        ...
        public void ProjectSetup_UsesAssemblyDefinitionsAndConfigurationDefaultsCapableRelease()
        {
            ...
            Assert.That(entry.Tag, Is.EqualTo("project-setup-v1.8.0"));
            Assert.That(entry.Summary, Does.Contain("project folders").And.Contain("assembly definitions").And.Contain("code generation defaults").And.Contain("duplicate naming defaults").And.Contain("scripting define symbols").And.Contain("Tags").And.Contain("Layers").And.Contain("Build Scenes").And.Contain("Play Mode Start Scene"));
        }
    }
}
