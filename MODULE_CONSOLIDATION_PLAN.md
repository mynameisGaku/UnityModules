...
**4. asmdef 依存の可視化とasmref target監査（AssemblyDependencyAudit 1.1.0で実装）**
...
1.1.0では`.asmref`を別一覧へ追加し、不正JSON、欠落・未解決・曖昧なtargetを検査する。`.asmref`はscriptの所属を指定するassetであり、asmdef依存graphへedgeを追加しない。
