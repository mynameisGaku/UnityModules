...
**4. asmdef依存・asmref target・folder owner候補の監査（AssemblyDependencyAudit 1.5.0で実装）**
...
1.5.0では選択asmdef／asmrefに関係するissueをresult順の500件pageで、既存上限50,000件に対応する最大100 pageまで全て到達可能にした。同じresult indexは1回にまとめるが、内容が同じ別issueは保持する。page変更では先頭issueを選択してIssue／Detailsのscrollを戻し、selection、Refresh、result clear、filter後の選択維持／置換に応じてpageをresetまたはclampする。不正な関連cacheはpartial resultを表示しない。Analyzer、Service、model、graph、issue taxonomy、既存Copy、公開API、Runtime、build callbackは変更しない。
