...
利用側の getter、setter、`Action` が例外を送出した場合は、その行だけを注意色の `ERROR` 表示へ切り替え、例外全文を Console と行のツールチップへ出します。`HasError`、`ErrorText`、`ErrorMessage` から状態を取得できます。値設定に失敗した行はチェック、スライダー、直接入力などの操作点を残すため、利用側の状態が回復した後に同じUIから再試行できます。成功した再試行はエラー表示を解除し、失敗した書き込みは変更通知・履歴・最近項目へ含めません。
...
Text / JSON からBool値を読む場合は `1` / `0` / `true` / `false` のみを受け付けます。`true` / `false` の大文字小文字は区別せず、前後の空白や別名は壊れた値として無視します。
...
`Settings`ページではプロファイル名と形式を指定して現在値を保存・適用できます。保存済みプロファイルは一覧から即時適用できます。File欄には任意パスを入力でき、`Save As`は選択形式で書き出し、`Load From`は拡張子に依存せず内容から形式を判別します。`Copy Menu Text`は現在の全ページをクリップボードへコピーし、各操作の成否は画面上の短い通知でも確認できます。`Recent`ページには直近16件の変更が重複なし・新しい順で並び、元の行と同じ実体をそのまま操作できます。実行中に行や子ページを追加・削除・移動した場合も、`Refresh()` が所有範囲を更新して孤立項目を除きます。`DebugMenuHistory.Clear()` は Undo / Redo と保留中の変更を破棄し、完了時点の現在値を次の変更の基準にします。

`Reset All` は `IsSaveable == true` かつ値の種類を持つ行だけを、借用表示があっても1実体につき1回復元します。配列は各 index、Vector は親の値だけが対象です。部分失敗時は後続行を継続し、成功件数と失敗件数を通知します。
