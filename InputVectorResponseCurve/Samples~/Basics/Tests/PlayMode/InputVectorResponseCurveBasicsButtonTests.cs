...
namespace InputResponse.Samples.PlayMode.Tests
{
    /// <summary>import済みBasicsの実Button、curve結果、responsive geometryを検証する。</summary>
    [Parallelizable(ParallelScope.None)]
    [UnityPlatform(RuntimePlatform.WindowsEditor, RuntimePlatform.OSXEditor, RuntimePlatform.LinuxEditor)]
    public sealed class InputVectorResponseCurveBasicsButtonTests
    {
        private const string PanelSettingsGuid = "fb000000000000000000000000000010";
        private GameObject _host;
        private UIDocument _document;
        private InputVectorResponseCurveBasicsController _sample;
        ...
        public IEnumerator SetUp()
        {
            ...
            _host = new GameObject("Input Vector Response Curve Basics Test Host");
            _host.SetActive(false);
            _document = _host.AddComponent<UIDocument>();
            _document.panelSettings = _panelSettings;
            _sample = _host.AddComponent<InputVectorResponseCurveBasicsController>();
            _host.SetActive(true);
            yield return WaitUntil(() => ReadyRoot()?.Q<VisualElement>(InputVectorResponseCurveBasicsController.CardElementName)?.worldBound.width > 0f, "960x600の実panelとsampleが準備されませんでした。");
            yield return null;
        }
        ...
        public IEnumerator InitialState_IsZeroAndHealthy()
        {
            ...
            Assert.That(_sample.LastMode, Is.EqualTo(InputVectorResponseMode.Linear));
            Assert.That(_sample.LastError, Is.EqualTo(InputVectorResponseCurveError.None));
            Assert.That(_sample.OverRangeRejected, Is.False);
            Assert.That(_sample.ButtonActionCount, Is.Zero);
            yield return null;
        }

        [UnityTest]
        public IEnumerator CurveButtons_ProduceGoldenDirectionPreservingOutputs()
        {
            Click(InputVectorResponseCurveBasicsController.LinearButtonElementName);
            AssertOutput(0.3d, 0.4d, 0.5d);
            Click(InputVectorResponseCurveBasicsController.SquaredButtonElementName);
            AssertOutput(0.15d, 0.2d, 0.25d);
            Click(InputVectorResponseCurveBasicsController.CubicButtonElementName);
            AssertOutput(0.075d, 0.1d, 0.125d);
            Click(InputVectorResponseCurveBasicsController.SmoothStepButtonElementName);
            AssertOutput(0.3d, 0.4d, 0.5d);
            Assert.That(_sample.LastHorizontal, Is.EqualTo(0.3d));
            Assert.That(_sample.LastVertical, Is.EqualTo(0.4d));
            Assert.That(_sample.LastMode, Is.EqualTo(InputVectorResponseMode.SmoothStep));
            Assert.That(_sample.LastError, Is.EqualTo(InputVectorResponseCurveError.None));
        ...
        [UnityTest]
        public IEnumerator OverRangeButton_PreservesLastSuccessfulOutput()
        {
            Click(InputVectorResponseCurveBasicsController.CubicButtonElementName);
            ...
            Click(InputVectorResponseCurveBasicsController.RejectOverRangeButtonElementName);
            Assert.That(_sample.LastHorizontal, Is.EqualTo(1d));
            Assert.That(_sample.LastVertical, Is.EqualTo(1d));
            ...
            Assert.That(_sample.LastError, Is.EqualTo(InputVectorResponseCurveError.InputOutOfRange));
            Assert.That(_sample.OverRangeRejected, Is.True);
        ...
        {
            var root = ReadyRoot();
            var card = root.Q<VisualElement>(InputVectorResponseCurveBasicsController.CardElementName);
            var names = new[]
            {
                InputVectorResponseCurveBasicsController.TitleElementName,
                InputVectorResponseCurveBasicsController.DescriptionElementName,
                InputVectorResponseCurveBasicsController.ConfigurationElementName,
                InputVectorResponseCurveBasicsController.InputElementName,
                InputVectorResponseCurveBasicsController.StageElementName,
                InputVectorResponseCurveBasicsController.ResultElementName,
                InputVectorResponseCurveBasicsController.LinearButtonElementName,
                InputVectorResponseCurveBasicsController.SquaredButtonElementName,
                InputVectorResponseCurveBasicsController.CubicButtonElementName,
                InputVectorResponseCurveBasicsController.SmoothStepButtonElementName,
                InputVectorResponseCurveBasicsController.RejectOverRangeButtonElementName
            };
            ...
            var buttons = new[]
            {
                root.Q<Button>(InputVectorResponseCurveBasicsController.LinearButtonElementName),
                root.Q<Button>(InputVectorResponseCurveBasicsController.SquaredButtonElementName),
                root.Q<Button>(InputVectorResponseCurveBasicsController.CubicButtonElementName),
                root.Q<Button>(InputVectorResponseCurveBasicsController.SmoothStepButtonElementName),
                root.Q<Button>(InputVectorResponseCurveBasicsController.RejectOverRangeButtonElementName)
            };
            if (wide)
            {
        ...
        {
            var target = new RenderTexture(width, height, 0, RenderTextureFormat.ARGB32) { name = $"Input Vector Response Curve Test {width}x{height}" };
            target.Create();
            return target;
        }
