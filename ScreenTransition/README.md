...
Scene や画面内容を切り替える前後に、全画面フェードを毎回作り直す手間を減らします。Scene の読込自体は行わないため、「シーン切り替え」と必要なときだけ組み合わせられます。

## こんなときに使う

- Scene 切り替えやメニュー遷移の瞬間を暗転で隠したい。
- Pause 中でもフェードを完了させたい。
- 二重実行や無効化時の後始末を各画面で実装したくない。
