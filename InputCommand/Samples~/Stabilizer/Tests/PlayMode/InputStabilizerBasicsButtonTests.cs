...
namespace InputStabilization.Samples.PlayMode.Tests
{
    /// <summary>import済みBasicsの実Button、連続sample契約、responsive geometryを検証する。</summary>
    [Parallelizable(ParallelScope.None)]
    [UnityPlatform(RuntimePlatform.WindowsEditor, RuntimePlatform.OSXEditor, RuntimePlatform.LinuxEditor)]
    public sealed class InputStabilizerBasicsButtonTests
    {
        private const string PanelSettingsGuid = "c8000000000000000000000000000010";
        private GameObject _host;
        private UIDocument _document;
        private InputStabilizerBasicsController _sample;
        ...
        public IEnumerator SetUp()
        {
            ...
            _host = new GameObject("Input Stabilizer Basics Test Host");
            _host.SetActive(false);
            _document = _host.AddComponent<UIDocument>();
            _document.panelSettings = _panelSettings;
            _sample = _host.AddComponent<InputStabilizerBasicsController>();
            _host.SetActive(true);
            yield return WaitUntil(() => ReadyRoot()?.Q<VisualElement>(InputStabilizerBasicsController.CardElementName)?.worldBound.width > 0f, "960x600の実panelとsampleが準備されませんでした。");
            yield return null;
        }
        ...
        /// <summary>初期状態がcommand 0、候補なし、操作なしで安定する。</summary>
        [UnityTest]
        public IEnumerator InitialState_IsZeroAndHealthy()
        {
            Assert.That(_sample.CurrentCommand, Is.Zero);
            Assert.That(_sample.CandidateCommand, Is.Zero);
            Assert.That(_sample.CandidateSampleCount, Is.Zero);
            Assert.That(_sample.LastChanged, Is.False);
            Assert.That(_sample.NoiseCancelled, Is.False);
            Assert.That(_sample.ButtonActionCount, Is.Zero);
            yield return null;
        }

        /// <summary>+4が3回目だけ確定し、それ以前は候補として残る。</summary>
        [UnityTest]
        public IEnumerator GoldenButtons_CommitOnlyOnThirdConsecutiveSample()
        {
            Click(InputStabilizerBasicsController.FirstSampleButtonElementName);
            Assert.That(_sample.CurrentCommand, Is.Zero);
            Assert.That(_sample.CandidateCommand, Is.EqualTo(4));
            Assert.That(_sample.CandidateSampleCount, Is.EqualTo(1));
            Click(InputStabilizerBasicsController.SecondSampleButtonElementName);
            Assert.That(_sample.CurrentCommand, Is.Zero);
            Assert.That(_sample.CandidateSampleCount, Is.EqualTo(2));
            Click(InputStabilizerBasicsController.CommitSampleButtonElementName);
            Assert.That(_sample.CurrentCommand, Is.EqualTo(4));
            Assert.That(_sample.CandidateSampleCount, Is.Zero);
            Assert.That(_sample.LastChanged, Is.True);
            Assert.That(_sample.ButtonActionCount, Is.EqualTo(3));
            yield return null;
        }

        /// <summary>1回だけの-4候補が現在+4へ戻るsampleで取り消される。</summary>
        [UnityTest]
        public IEnumerator NoiseButton_IsCancelledByCurrentCommand()
        {
            Click(InputStabilizerBasicsController.FirstSampleButtonElementName);
            Click(InputStabilizerBasicsController.SecondSampleButtonElementName);
            Click(InputStabilizerBasicsController.CommitSampleButtonElementName);
            Click(InputStabilizerBasicsController.NoiseButtonElementName);
            Assert.That(_sample.CurrentCommand, Is.EqualTo(4));
            Assert.That(_sample.CandidateCommand, Is.EqualTo(-4));
            Assert.That(_sample.CandidateSampleCount, Is.EqualTo(1));
            Click(InputStabilizerBasicsController.CancelNoiseButtonElementName);
            Assert.That(_sample.CurrentCommand, Is.EqualTo(4));
            Assert.That(_sample.CandidateSampleCount, Is.Zero);
            Assert.That(_sample.NoiseCancelled, Is.True);
        ...
        {
            var root = ReadyRoot();
            var card = root.Q<VisualElement>(InputStabilizerBasicsController.CardElementName);
            var names = new[]
            {
                InputStabilizerBasicsController.TitleElementName,
                InputStabilizerBasicsController.DescriptionElementName,
                InputStabilizerBasicsController.ConfigurationElementName,
                InputStabilizerBasicsController.InputElementName,
                InputStabilizerBasicsController.StageElementName,
                InputStabilizerBasicsController.ResultElementName,
                InputStabilizerBasicsController.FirstSampleButtonElementName,
                InputStabilizerBasicsController.SecondSampleButtonElementName,
                InputStabilizerBasicsController.CommitSampleButtonElementName,
                InputStabilizerBasicsController.NoiseButtonElementName,
                InputStabilizerBasicsController.CancelNoiseButtonElementName
            };
            ...
            var buttons = new[]
            {
                root.Q<Button>(InputStabilizerBasicsController.FirstSampleButtonElementName),
                root.Q<Button>(InputStabilizerBasicsController.SecondSampleButtonElementName),
                root.Q<Button>(InputStabilizerBasicsController.CommitSampleButtonElementName),
                root.Q<Button>(InputStabilizerBasicsController.NoiseButtonElementName),
                root.Q<Button>(InputStabilizerBasicsController.CancelNoiseButtonElementName)
            };
            if (wide)
            {
        ...
        {
            var target = new RenderTexture(width, height, 0, RenderTextureFormat.ARGB32) { name = $"Input Stabilizer Test {width}x{height}" };
            target.Create();
            return target;
        }
