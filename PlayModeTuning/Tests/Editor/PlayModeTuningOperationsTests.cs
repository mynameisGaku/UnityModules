...
namespace PlayModeTuning.Editor.Tests
{
    public sealed class PlayModeTuningOperationsTests
    {
        ...
        public void FullManualFlowAppliesCapturedValueAndMarksSceneDirty()
        {
            ...
            Assert.That(flow.Gateway.CompleteApplyCalls, Is.EqualTo(1));
            Assert.That(flow.Gateway.ReleaseApplyCalls, Is.EqualTo(1));
            Assert.That(flow.Gateway.RevertApplyCalls, Is.Zero);
        ...
        public void OnValidateUnselectedSideEffectFailsPostVerificationAndResidualFailsRollback()
        {
            ...
            Assert.That(flow.Gateway.RevertApplyCalls, Is.EqualTo(1));
        ...
        [Test]
        public void UnexpectedSaveFailureAfterMutationRollsBackAndHidesDetail()
        {
            const string secretDetail = "PRIVATE_SAVE_PATH_DO_NOT_DISPLAY";
            var flow = ReadyPlan(1f, 5f, out var plan);
            flow.Store.SaveFailureDetail = secretDetail;
            flow.Store.SaveFailureCall = flow.Store.SaveCalls + 2;
            LogAssert.Expect(LogType.Exception, new Regex("InvalidOperationException: " + secretDetail));

            var result = flow.Operations.Apply(plan);

            Assert.That(result.ApplySucceeded, Is.False);
            Assert.That(result.ApplyError, Is.EqualTo(PlayModeTuningError.SessionStorageFailed));
            Assert.That(result.ApplyMessage, Does.Not.Contain(secretDetail));
            Assert.That(result.RollbackSucceeded, Is.True);
            Assert.That(flow.Gateway.RevertApplyCalls, Is.EqualTo(1));
            Assert.That(flow.Gateway.GetValue("c0", "speed").EqualsExact(FakePlayModeTuningGateway.FloatValue(1f)), Is.True);
        }
        ...
        [Test]
        public void PlanConsumptionSaveFailureAllowsSafeRetryBeforeMutation()
        {
            const string secretDetail = "PRIVATE_CONSUME_PATH_DO_NOT_DISPLAY";
            var flow = ReadyPlan(1f, 5f, out var plan);
            flow.Store.SaveFailureDetail = secretDetail;
            flow.Store.SaveFailureCall = flow.Store.SaveCalls + 1;
            LogAssert.Expect(LogType.Exception, new Regex("InvalidOperationException: " + secretDetail));

            var failed = flow.Operations.Apply(plan);

            Assert.That(failed.ApplyAttempted, Is.False);
            Assert.That(failed.ApplyError, Is.EqualTo(PlayModeTuningError.SessionStorageFailed));
            Assert.That(failed.ApplyMessage, Does.Not.Contain(secretDetail));
            Assert.That(flow.Gateway.ApplyCalls, Is.Zero);
            flow.Store.SaveFailureCall = 0;

            var retried = flow.Operations.Apply(plan);

            Assert.That(retried.ApplySucceeded, Is.True);
            Assert.That(flow.Gateway.GetValue("c0", "speed").EqualsExact(FakePlayModeTuningGateway.FloatValue(5f)), Is.True);
        }

        [Test]
        public void PlanLoadFailureAllowsSafeRetryBeforeMutation()
        {
            const string secretDetail = "PRIVATE_PLAN_LOAD_DO_NOT_DISPLAY";
            var flow = ReadyPlan(1f, 5f, out var plan);
        ...
        [Test]
        public void StartSaveFailureReturnsResultWithoutLeakingDetail()
        {
            const string secretDetail = "PRIVATE_START_SAVE_DO_NOT_DISPLAY";
            var flow = new PlayModeTuningTestFlow();
            flow.Store.SaveFailureCall = 1;
            flow.Store.SaveFailureDetail = secretDetail;
            LogAssert.Expect(LogType.Exception, new Regex("InvalidOperationException: " + secretDetail));

            var result = flow.Operations.Start(new[] { FakePlayModeTuningGateway.Selection("c0", "speed") });

            Assert.That(result.Error, Is.EqualTo(PlayModeTuningError.SessionStorageFailed));
            Assert.That(result.Message, Does.Not.Contain(secretDetail));
            Assert.That(flow.Store.Current, Is.Null);
            Assert.That(flow.Gateway.ApplyCalls, Is.Zero);
        }

        [Test]
        public void CaptureSaveFailurePreservesCapturableStoredSession()
        {
            const string secretDetail = "PRIVATE_CAPTURE_SAVE_DO_NOT_DISPLAY";
            var flow = new PlayModeTuningTestFlow();
            flow.Start(FakePlayModeTuningGateway.Selection("c0", "speed"));
            flow.EnterPlay();
            ...
            Assert.That(result.Error, Is.EqualTo(PlayModeTuningError.SessionStorageFailed));
            Assert.That(result.Message, Does.Not.Contain(secretDetail));
            Assert.That(result.Session.Phase, Is.EqualTo(PlayModeTuningPhase.Capturable));
            Assert.That((PlayModeTuningPhase)flow.Store.Current.phase, Is.EqualTo(PlayModeTuningPhase.Capturable));
        }

        [Test]
        public void PreviewSaveFailureKeepsRetryableStoredSession()
        {
            const string secretDetail = "PRIVATE_PREVIEW_SAVE_DO_NOT_DISPLAY";
            var flow = ReadyForPreview(1f, 2f);
            flow.Store.SaveFailureCall = flow.Store.SaveCalls + 1;
            flow.Store.SaveFailureDetail = secretDetail;
            LogAssert.Expect(LogType.Exception, new Regex("InvalidOperationException: " + secretDetail));

            var failed = flow.Preview();

            Assert.That(failed.Error, Is.EqualTo(PlayModeTuningError.SessionStorageFailed));
            Assert.That(failed.Message, Does.Not.Contain(secretDetail));
            Assert.That((PlayModeTuningPhase)flow.Store.Current.phase, Is.EqualTo(PlayModeTuningPhase.ReadyToPreview));
            flow.Store.SaveFailureCall = 0;
            Assert.That(flow.Preview().IsReady, Is.True);
        }

        [Test]
        public void DiscardSaveFailureReportsErrorAndPreservesActiveSession()
        {
            const string secretDetail = "PRIVATE_DISCARD_SAVE_DO_NOT_DISPLAY";
            var flow = new PlayModeTuningTestFlow();
            flow.Start(FakePlayModeTuningGateway.Selection("c0", "speed"));
            flow.Store.SaveFailureCall = flow.Store.SaveCalls + 1;
            flow.Store.SaveFailureDetail = secretDetail;
            ...
            var result = flow.Operations.Discard(flow.SessionId);

            Assert.That(result.Error, Is.EqualTo(PlayModeTuningError.SessionStorageFailed));
            Assert.That(result.Message, Does.Not.Contain(secretDetail));
            Assert.That(result.Phase, Is.EqualTo(PlayModeTuningPhase.Armed));
            Assert.That((PlayModeTuningPhase)flow.Store.Current.phase, Is.EqualTo(PlayModeTuningPhase.Armed));
        }

        [Test]
        public void StaleSaveFailureReturnsStorageErrorAndKeepsPreviousPhase()
        {
            const string secretDetail = "PRIVATE_STALE_SAVE_DO_NOT_DISPLAY";
            var flow = new PlayModeTuningTestFlow();
            flow.Start(FakePlayModeTuningGateway.Selection("c0", "speed"));
            flow.EnterPlay();
            flow.Gateway.FailCapture = true;
            flow.Store.SaveFailureCall = flow.Store.SaveCalls + 1;
            flow.Store.SaveFailureDetail = secretDetail;
            LogAssert.Expect(LogType.Exception, new Regex("InvalidOperationException: " + secretDetail));
        ...
        [Test]
        public void LifecycleSaveFailureIsLoggedAndLeavesStoredPhaseUnchanged()
        {
            const string secretDetail = "PRIVATE_LIFECYCLE_SAVE_DO_NOT_DISPLAY";
            var flow = new PlayModeTuningTestFlow();
            flow.Start(FakePlayModeTuningGateway.Selection("c0", "speed"));
            flow.Store.SaveFailureCall = flow.Store.SaveCalls + 1;
            flow.Store.SaveFailureDetail = secretDetail;
            ...
            Assert.That(flow.Operations.ResumeLifecycle(EPlayModeTuningObservedTransition.EnteredPlayMode), Is.True);
            Assert.That(flow.Operations.GetCurrentSession().Phase, Is.EqualTo(PlayModeTuningPhase.Capturable));
            Assert.That(flow.Operations.GetCurrentSession().Error, Is.EqualTo(PlayModeTuningError.None));
        }

        [Test]
        public void CompleteApplyFailureRollsBackAndPersistsFailureState()
        {
            var flow = ReadyPlan(1f, 5f, out var plan);
            flow.Gateway.FailCompleteApply = true;
            ...
            Assert.That(result.RollbackSucceeded, Is.True);
            Assert.That(flow.Gateway.CompleteApplyCalls, Is.EqualTo(1));
            Assert.That(flow.Gateway.ReleaseApplyCalls, Is.Zero);
            Assert.That(flow.Gateway.GetValue("c0", "speed").EqualsExact(FakePlayModeTuningGateway.FloatValue(1f)), Is.True);
            Assert.That((PlayModeTuningPhase)flow.Store.Current.phase, Is.EqualTo(PlayModeTuningPhase.Completed));
            Assert.That((PlayModeTuningError)flow.Store.Current.error, Is.EqualTo(PlayModeTuningError.ApplyFailed));
        }

        [Test]
        public void FinalSaveAndEveryCleanupFailureCannotLeaveStoredSuccessAfterCompletedUndo()
        {
            const string saveDetail = "PRIVATE_FINAL_AND_CLEANUP_SAVE_DO_NOT_DISPLAY";
            const string clearDetail = "PRIVATE_FINAL_CLEAR_DO_NOT_DISPLAY";
            var flow = ReadyPlan(1f, 5f, out var plan);
            flow.Store.SaveFailureDetail = saveDetail;
            flow.Store.ClearFailure = true;
            flow.Store.ClearFailureDetail = clearDetail;
            flow.Store.SaveFailureCalls.Add(flow.Store.SaveCalls + 3);
            flow.Store.SaveFailureCalls.Add(flow.Store.SaveCalls + 4);
            flow.Store.SaveFailureCalls.Add(flow.Store.SaveCalls + 5);
            LogAssert.Expect(LogType.Exception, new Regex("InvalidOperationException: " + saveDetail));
            LogAssert.Expect(LogType.Exception, new Regex("InvalidOperationException: " + saveDetail));
            LogAssert.Expect(LogType.Exception, new Regex("InvalidOperationException: " + clearDetail));
            LogAssert.Expect(LogType.Exception, new Regex("InvalidOperationException: " + saveDetail));
            ...
            Assert.That(result.ApplySucceeded, Is.False);
            Assert.That(result.ApplyError, Is.EqualTo(PlayModeTuningError.SessionStorageFailed));
            Assert.That(result.ApplyMessage, Does.Not.Contain(saveDetail));
            Assert.That(result.ApplyMessage, Does.Not.Contain(clearDetail));
            Assert.That(result.RollbackSucceeded, Is.True);
            Assert.That(flow.Gateway.CompleteApplyCalls, Is.EqualTo(1));
            Assert.That(flow.Gateway.ReleaseApplyCalls, Is.Zero);
            Assert.That(flow.Gateway.GetValue("c0", "speed").EqualsExact(FakePlayModeTuningGateway.FloatValue(1f)), Is.True);
            Assert.That((PlayModeTuningPhase)flow.Store.Current.phase, Is.EqualTo(PlayModeTuningPhase.Stale));
            Assert.That((PlayModeTuningError)flow.Store.Current.error, Is.EqualTo(PlayModeTuningError.SessionStorageFailed));
            Assert.That(flow.Store.Current.message, Does.Contain("対象シーンと取り消し履歴を確認"));
            var afterReload = new PlayModeTuningOperations(flow.Gateway, flow.Store, new PlayModeTuningPlanRegistry(), "domain-c").GetCurrentSession();
            Assert.That(afterReload.Phase, Is.EqualTo(PlayModeTuningPhase.Stale));
            Assert.That(afterReload.Error, Is.EqualTo(PlayModeTuningError.SessionStorageFailed));
        }

        [Test]
        public void DoubleTerminalSaveFailureClearsMisleadingStoredStateAfterRollback()
        {
            const string secretDetail = "PRIVATE_DOUBLE_SAVE_DO_NOT_DISPLAY";
            var flow = ReadyPlan(1f, 5f, out var plan);
            flow.Store.SaveFailureDetail = secretDetail;
            flow.Store.SaveFailureCalls.Add(flow.Store.SaveCalls + 2);
            flow.Store.SaveFailureCalls.Add(flow.Store.SaveCalls + 3);
            LogAssert.Expect(LogType.Exception, new Regex("InvalidOperationException: " + secretDetail));
            LogAssert.Expect(LogType.Exception, new Regex("InvalidOperationException: " + secretDetail));
            ...
            Assert.That(result.ApplyError, Is.EqualTo(PlayModeTuningError.SessionStorageFailed));
            Assert.That(result.ApplyMessage, Does.Not.Contain(secretDetail));
            Assert.That(result.ApplyMessage, Does.Contain("消去しました"));
            Assert.That(result.RollbackSucceeded, Is.True);
            Assert.That(flow.Gateway.GetValue("c0", "speed").EqualsExact(FakePlayModeTuningGateway.FloatValue(1f)), Is.True);
            Assert.That(flow.Store.Current, Is.Null);
        }
        ...
        [Test]
        public void ClearFailureFallsBackToPersistedInvalidStateWithoutLeakingDetail()
        {
            const string saveDetail = "PRIVATE_DOUBLE_SAVE_FOR_CLEAR_DO_NOT_DISPLAY";
            const string clearDetail = "PRIVATE_CLEAR_DO_NOT_DISPLAY";
            var flow = ReadyPlan(1f, 5f, out var plan);
            flow.Store.SaveFailureDetail = saveDetail;
            flow.Store.SaveFailureCalls.Add(flow.Store.SaveCalls + 2);
            flow.Store.SaveFailureCalls.Add(flow.Store.SaveCalls + 3);
            ...
            Assert.That(result.ApplyMessage, Does.Not.Contain(saveDetail));
            Assert.That(result.ApplyMessage, Does.Not.Contain(clearDetail));
            Assert.That(result.ApplyMessage, Does.Contain("無効状態"));
            Assert.That(result.RollbackSucceeded, Is.True);
            Assert.That((PlayModeTuningPhase)flow.Store.Current.phase, Is.EqualTo(PlayModeTuningPhase.Stale));
            Assert.That((PlayModeTuningError)flow.Store.Current.error, Is.EqualTo(PlayModeTuningError.SessionStorageFailed));
            Assert.That(result.Session.Message, Is.EqualTo(result.ApplyMessage));
            Assert.That(flow.Store.Current.message, Is.EqualTo(result.ApplyMessage));
            Assert.That(flow.Operations.GetCurrentSession().Message, Is.EqualTo(result.ApplyMessage));
        }
