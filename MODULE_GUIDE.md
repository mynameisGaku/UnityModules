...
| 入力コマンド判定 | Input Command | 先行入力・順序・同時押し・優先順位をtick基準で判定する。 |
...
公開済みタグと UPM 識別子は削除せず、既存利用者の互換入口として残す。統合後の推奨入口は次のとおり。
...
いずれも `ulong tick` と `int commandId` を扱うため、一つの assembly に置いて初めて pipeline として繋がる。
