...
{
    /// <summary>import済みBasicsの実Button、方向回転制限結果、responsive geometryを検証する。</summary>
    [Parallelizable(ParallelScope.None)]
    [UnityPlatform(RuntimePlatform.WindowsEditor, RuntimePlatform.OSXEditor, RuntimePlatform.LinuxEditor)]
    public sealed class InputVectorDirectionLimiterBasicsButtonTests
    {
        private const string PanelSettingsGuid = "fc100000000000000000000000000010";
        private GameObject _host;
        private UIDocument _document;
        private InputVectorDirectionLimiterBasicsController _sample;
        ...
        public IEnumerator SetUp()
        {
            ...
            _host = new GameObject("Input Vector Direction Limiter Basics Test Host");
            _host.SetActive(false);
            _document = _host.AddComponent<UIDocument>();
            _document.panelSettings = _panelSettings;
            _sample = _host.AddComponent<InputVectorDirectionLimiterBasicsController>();
            _host.SetActive(true);
            yield return WaitUntil(() => ReadyRoot()?.Q<VisualElement>(InputVectorDirectionLimiterBasicsController.CardElementName)?.worldBound.width > 0f, "960x600の実panelとsampleが準備されませんでした。");
            yield return null;
        }
        ...
        {
            AssertCurrent(1d, 0d);
            Assert.That(_sample.CurrentMaximumTurnRadians, Is.EqualTo(Math.PI / 4d));
            Assert.That(_sample.LastResult.Succeeded, Is.True);
            Assert.That(_sample.LastResult.ReachedTargetDirection, Is.True);
            Assert.That(_sample.ButtonActionCount, Is.Zero);
            yield return null;
        }

        [UnityTest]
        public IEnumerator ScenarioButtons_ProduceGoldenDirectionStates()
        {
            Click(InputVectorDirectionLimiterBasicsController.TurnStepButtonElementName);
            var unitDiagonal = Math.Sqrt(0.5d);
            AssertCurrent(unitDiagonal, unitDiagonal);
            AssertResult(Math.PI / 4d, Math.PI / 4d, false);
            Click(InputVectorDirectionLimiterBasicsController.TurnAgainButtonElementName);
            AssertCurrent(0d, 1d);
            AssertResult(Math.PI / 4d, 0d, true);
            Click(InputVectorDirectionLimiterBasicsController.ClockwiseButtonElementName);
            var halfDiagonal = Math.Sqrt(0.125d);
            AssertCurrent(halfDiagonal, -halfDiagonal);
            Assert.That(_sample.LastResult.TargetMagnitude, Is.EqualTo(0.5d));
            AssertResult(Math.PI / 4d, Math.PI / 4d, false);
            Click(InputVectorDirectionLimiterBasicsController.OppositeButtonElementName);
            AssertCurrent(0d, 1d);
            Assert.That(_sample.CurrentMaximumTurnRadians, Is.EqualTo(Math.PI / 2d));
            AssertResult(Math.PI / 2d, Math.PI / 2d, false);
        ...
        public IEnumerator RejectButton_PreservesLastSuccessfulState()
        {
            ...
            Click(InputVectorDirectionLimiterBasicsController.RejectButtonElementName);
            AssertCurrent(beforeHorizontal, beforeVertical);
            Assert.That(_sample.LastTargetHorizontal, Is.EqualTo(0.8d));
            Assert.That(_sample.LastTargetVertical, Is.EqualTo(0.8d));
            Assert.That(_sample.LastResult.Succeeded, Is.False);
            Assert.That(_sample.LastResult.Error, Is.EqualTo(InputVectorDirectionLimiterError.InputOutsideUnitCircle));
        ...
        {
            var root = ReadyRoot();
            var card = root.Q<VisualElement>(InputVectorDirectionLimiterBasicsController.CardElementName);
            var names = new[]
            {
                InputVectorDirectionLimiterBasicsController.TitleElementName,
                InputVectorDirectionLimiterBasicsController.DescriptionElementName,
                InputVectorDirectionLimiterBasicsController.ConfigurationElementName,
                InputVectorDirectionLimiterBasicsController.InputElementName,
                InputVectorDirectionLimiterBasicsController.StageElementName,
                InputVectorDirectionLimiterBasicsController.ResultElementName,
                InputVectorDirectionLimiterBasicsController.TurnStepButtonElementName,
                InputVectorDirectionLimiterBasicsController.TurnAgainButtonElementName,
                InputVectorDirectionLimiterBasicsController.ClockwiseButtonElementName,
                InputVectorDirectionLimiterBasicsController.OppositeButtonElementName,
                InputVectorDirectionLimiterBasicsController.RejectButtonElementName
            };
            ...
            var buttons = new[]
            {
                root.Q<Button>(InputVectorDirectionLimiterBasicsController.TurnStepButtonElementName),
                root.Q<Button>(InputVectorDirectionLimiterBasicsController.TurnAgainButtonElementName),
                root.Q<Button>(InputVectorDirectionLimiterBasicsController.ClockwiseButtonElementName),
                root.Q<Button>(InputVectorDirectionLimiterBasicsController.OppositeButtonElementName),
                root.Q<Button>(InputVectorDirectionLimiterBasicsController.RejectButtonElementName)
            };
            if (wide)
            {
        ...
        {
            var target = new RenderTexture(width, height, 0, RenderTextureFormat.ARGB32) { name = $"Input Vector Direction Limiter Test {width}x{height}" };
            target.Create();
            return target;
        }
        ...
        {
            Assert.That(_sample.LastResult.Succeeded, Is.True);
            Assert.That(_sample.LastResult.AppliedTurnRadians, Is.EqualTo(applied).Within(1e-12d));
            Assert.That(_sample.LastResult.RemainingTurnRadians, Is.EqualTo(remaining).Within(1e-12d));
            Assert.That(_sample.LastResult.ReachedTargetDirection, Is.EqualTo(reached));
        }
    }
}
