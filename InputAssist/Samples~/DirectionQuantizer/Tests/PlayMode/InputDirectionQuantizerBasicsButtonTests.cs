...
namespace InputDirectionQuantization.Samples.PlayMode.Tests
{
    /// <summary>import済みBasicsの実Button、量子化結果、responsive geometryを検証する。</summary>
    [Parallelizable(ParallelScope.None)]
    [UnityPlatform(RuntimePlatform.WindowsEditor, RuntimePlatform.OSXEditor, RuntimePlatform.LinuxEditor)]
    public sealed class InputDirectionQuantizerBasicsButtonTests
    {
        private const string PanelSettingsGuid = "f6000000000000000000000000000010";
        private GameObject _host;
        private UIDocument _document;
        private InputDirectionQuantizerBasicsController _sample;
        ...
        public IEnumerator SetUp()
        {
            ...
            _host = new GameObject("Input Direction Quantizer Basics Test Host");
            _host.SetActive(false);
            _document = _host.AddComponent<UIDocument>();
            _document.panelSettings = _panelSettings;
            _sample = _host.AddComponent<InputDirectionQuantizerBasicsController>();
            _host.SetActive(true);
            yield return WaitUntil(() => ReadyRoot()?.Q<VisualElement>(InputDirectionQuantizerBasicsController.CardElementName)?.worldBound.width > 0f, "960x600の実panelとsampleが準備されませんでした。");
            yield return null;
        }
        ...
        [UnityTest]
        public IEnumerator InitialState_IsNeutralEightWayAndHealthy()
        {
            Assert.That(_sample.CurrentHorizontal, Is.Zero);
            Assert.That(_sample.CurrentVertical, Is.Zero);
            Assert.That(_sample.LastHorizontal, Is.Zero);
            Assert.That(_sample.LastVertical, Is.Zero);
            Assert.That(_sample.LastMode, Is.EqualTo(InputDirectionMode.EightWay));
            Assert.That(_sample.LastError, Is.EqualTo(InputDirectionQuantizationError.None));
            Assert.That(_sample.NonFiniteRejected, Is.False);
            Assert.That(_sample.ButtonActionCount, Is.Zero);
            yield return null;
        }

        /// <summary>実Button列がradial neutral、right、diagonal、4-way tieのgolden sequenceへ到達する。</summary>
        [UnityTest]
        public IEnumerator GoldenButtons_ProduceFourWayAndEightWayDirections()
        {
            Click(InputDirectionQuantizerBasicsController.DeadZoneButtonElementName);
            AssertDirection(0, 0);
            Click(InputDirectionQuantizerBasicsController.RightButtonElementName);
            AssertDirection(1, 0);
            Click(InputDirectionQuantizerBasicsController.DiagonalButtonElementName);
            AssertDirection(-1, 1);
            Click(InputDirectionQuantizerBasicsController.FourWayTieButtonElementName);
            AssertDirection(0, -1);
            Assert.That(_sample.LastHorizontal, Is.EqualTo(0.5d));
            Assert.That(_sample.LastVertical, Is.EqualTo(-0.5d));
            Assert.That(_sample.LastMode, Is.EqualTo(InputDirectionMode.FourWay));
            Assert.That(_sample.LastError, Is.EqualTo(InputDirectionQuantizationError.None));
        ...
        public IEnumerator NonFiniteButton_PreservesLastSuccessfulValue()
        {
            ...
            Click(InputDirectionQuantizerBasicsController.RejectNonFiniteButtonElementName);
            ...
            Assert.That(_sample.LastError, Is.EqualTo(InputDirectionQuantizationError.NonFiniteInput));
        ...
        {
            var root = ReadyRoot();
            var card = root.Q<VisualElement>(InputDirectionQuantizerBasicsController.CardElementName);
            var names = new[]
            {
                InputDirectionQuantizerBasicsController.TitleElementName,
                InputDirectionQuantizerBasicsController.DescriptionElementName,
                InputDirectionQuantizerBasicsController.ConfigurationElementName,
                InputDirectionQuantizerBasicsController.InputElementName,
                InputDirectionQuantizerBasicsController.StageElementName,
                InputDirectionQuantizerBasicsController.ResultElementName,
                InputDirectionQuantizerBasicsController.DeadZoneButtonElementName,
                InputDirectionQuantizerBasicsController.RightButtonElementName,
                InputDirectionQuantizerBasicsController.DiagonalButtonElementName,
                InputDirectionQuantizerBasicsController.FourWayTieButtonElementName,
                InputDirectionQuantizerBasicsController.RejectNonFiniteButtonElementName
            };
            ...
            var buttons = new[]
            {
                root.Q<Button>(InputDirectionQuantizerBasicsController.DeadZoneButtonElementName),
                root.Q<Button>(InputDirectionQuantizerBasicsController.RightButtonElementName),
                root.Q<Button>(InputDirectionQuantizerBasicsController.DiagonalButtonElementName),
                root.Q<Button>(InputDirectionQuantizerBasicsController.FourWayTieButtonElementName),
                root.Q<Button>(InputDirectionQuantizerBasicsController.RejectNonFiniteButtonElementName)
            };
            if (wide)
            {
        ...
        {
            var target = new RenderTexture(width, height, 0, RenderTextureFormat.ARGB32) { name = $"Input Direction Quantizer Test {width}x{height}" };
            target.Create();
            return target;
        }
        ...
        private void AssertDirection(int horizontal, int vertical)
        {
            Assert.That(_sample.CurrentHorizontal, Is.EqualTo(horizontal));
            Assert.That(_sample.CurrentVertical, Is.EqualTo(vertical));
        }
    }
}
