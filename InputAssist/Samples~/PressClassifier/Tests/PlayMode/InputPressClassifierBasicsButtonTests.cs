...
namespace InputPressing.Samples.PlayMode.Tests
{
    /// <summary>import済みBasicsの実Button、tap・hold契約、responsive geometryを検証する。</summary>
    [Parallelizable(ParallelScope.None)]
    [UnityPlatform(RuntimePlatform.WindowsEditor, RuntimePlatform.OSXEditor, RuntimePlatform.LinuxEditor)]
    public sealed class InputPressClassifierBasicsButtonTests
    {
        private const string PanelSettingsGuid = "ed000000000000000000000000000010";
        private GameObject _host;
        private UIDocument _document;
        private InputPressClassifierBasicsController _sample;
        ...
        public IEnumerator SetUp()
        {
            ...
            _host = new GameObject("Input Press Classifier Basics Test Host");
            _host.SetActive(false);
            _document = _host.AddComponent<UIDocument>();
            _document.panelSettings = _panelSettings;
            _sample = _host.AddComponent<InputPressClassifierBasicsController>();
            _host.SetActive(true);
            yield return WaitUntil(() => ReadyRoot()?.Q<VisualElement>(InputPressClassifierBasicsController.CardElementName)?.worldBound.width > 0f, "960x600の実panelとsampleが準備されませんでした。");
            yield return null;
        }
        ...
        /// <summary>初期tick、released状態、操作数が設定どおりで安定する。</summary>
        [UnityTest]
        public IEnumerator InitialState_IsReadyAtTick100()
        {
            ...
            Assert.That(_sample.IsHolding, Is.False);
            Assert.That(_sample.LastError, Is.EqualTo(InputPressError.None));
            Assert.That(_sample.ButtonActionCount, Is.Zero);
            yield return null;
        }

        /// <summary>2 tickの押下をtapへ分類する。</summary>
        [UnityTest]
        public IEnumerator TapPressAndRelease_ClassifiesTap()
        {
            Click(InputPressClassifierBasicsController.TapPressButtonElementName);
            Assert.That(_sample.LastPressStarted, Is.True);
            Assert.That(_sample.IsPressed, Is.True);
            Click(InputPressClassifierBasicsController.TapReleaseButtonElementName);
            Assert.That(_sample.CurrentTick, Is.EqualTo(102));
            Assert.That(_sample.LastReleased, Is.True);
            Assert.That(_sample.LastTapped, Is.True);
            Assert.That(_sample.LastHoldCompleted, Is.False);
            Assert.That(_sample.LastPressDurationTicks, Is.EqualTo(2));
            Assert.That(_sample.ButtonActionCount, Is.EqualTo(2));
            yield return null;
        }

        /// <summary>3 tickの閾値到達でholdを1回開始する。</summary>
        [UnityTest]
        public IEnumerator HoldPressAndCheck_StartsHold()
        {
            Click(InputPressClassifierBasicsController.TapPressButtonElementName);
            Click(InputPressClassifierBasicsController.TapReleaseButtonElementName);
            Click(InputPressClassifierBasicsController.HoldPressButtonElementName);
            Click(InputPressClassifierBasicsController.HoldThresholdButtonElementName);
            Assert.That(_sample.CurrentTick, Is.EqualTo(106));
            Assert.That(_sample.IsPressed, Is.True);
            Assert.That(_sample.IsHolding, Is.True);
            Assert.That(_sample.LastHoldStarted, Is.True);
            Assert.That(_sample.LastPressDurationTicks, Is.EqualTo(3));
            Assert.That(_sample.ButtonActionCount, Is.EqualTo(4));
            yield return null;
        }

        /// <summary>5操作でtapとhold完了を順に再現する。</summary>
        [UnityTest]
        public IEnumerator FiveButtons_CompleteTapThenHold()
        {
            Click(InputPressClassifierBasicsController.TapPressButtonElementName);
            Click(InputPressClassifierBasicsController.TapReleaseButtonElementName);
            Click(InputPressClassifierBasicsController.HoldPressButtonElementName);
            Click(InputPressClassifierBasicsController.HoldThresholdButtonElementName);
            Click(InputPressClassifierBasicsController.HoldReleaseButtonElementName);
            Assert.That(_sample.CurrentTick, Is.EqualTo(108));
            Assert.That(_sample.IsPressed, Is.False);
            Assert.That(_sample.IsHolding, Is.False);
            Assert.That(_sample.LastReleased, Is.True);
            Assert.That(_sample.LastTapped, Is.False);
            Assert.That(_sample.LastHoldCompleted, Is.True);
            Assert.That(_sample.LastPressDurationTicks, Is.EqualTo(5));
        ...
        {
            var root = ReadyRoot();
            var card = root.Q<VisualElement>(InputPressClassifierBasicsController.CardElementName);
            var names = new[]
            {
                InputPressClassifierBasicsController.TitleElementName,
                InputPressClassifierBasicsController.DescriptionElementName,
                InputPressClassifierBasicsController.ConfigurationElementName,
                InputPressClassifierBasicsController.InputElementName,
                InputPressClassifierBasicsController.StageElementName,
                InputPressClassifierBasicsController.ResultElementName,
                InputPressClassifierBasicsController.TapPressButtonElementName,
                InputPressClassifierBasicsController.TapReleaseButtonElementName,
                InputPressClassifierBasicsController.HoldPressButtonElementName,
                InputPressClassifierBasicsController.HoldThresholdButtonElementName,
                InputPressClassifierBasicsController.HoldReleaseButtonElementName
            };
            ...
            var buttons = new[]
            {
                root.Q<Button>(InputPressClassifierBasicsController.TapPressButtonElementName),
                root.Q<Button>(InputPressClassifierBasicsController.TapReleaseButtonElementName),
                root.Q<Button>(InputPressClassifierBasicsController.HoldPressButtonElementName),
                root.Q<Button>(InputPressClassifierBasicsController.HoldThresholdButtonElementName),
                root.Q<Button>(InputPressClassifierBasicsController.HoldReleaseButtonElementName)
            };
            if (wide)
            {
        ...
        {
            var target = new RenderTexture(width, height, 0, RenderTextureFormat.ARGB32) { name = $"Input Press Classifier Test {width}x{height}" };
            target.Create();
            return target;
        }
