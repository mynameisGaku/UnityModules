...
namespace InputQuantization.Samples
{
    /// <summary>dead zone、対称量子化、clamp、非有限値拒否を実Buttonで確認するサンプル。</summary>
    [RequireComponent(typeof(UIDocument))]
    public sealed class InputQuantizerBasicsController : MonoBehaviour
    {
        /// <summary>card要素名。</summary>
        public const string CardElementName = "input-quantizer-basics-card";

        /// <summary>title要素名。</summary>
        public const string TitleElementName = "input-quantizer-basics-title";

        /// <summary>説明要素名。</summary>
        public const string DescriptionElementName = "input-quantizer-basics-description";

        /// <summary>設定表示要素名。</summary>
        public const string ConfigurationElementName = "input-quantizer-basics-configuration";

        /// <summary>入力状態要素名。</summary>
        public const string InputElementName = "input-quantizer-basics-input";

        /// <summary>操作結果要素名。</summary>
        public const string StageElementName = "input-quantizer-basics-stage";

        /// <summary>量子化結果要素名。</summary>
        public const string ResultElementName = "input-quantizer-basics-result";

        /// <summary>Button列要素名。</summary>
        public const string ButtonRowElementName = "input-quantizer-basics-buttons";

        /// <summary>dead zone確認Button要素名。</summary>
        public const string DeadZoneButtonElementName = "input-quantizer-basics-dead-zone";

        /// <summary>正入力Button要素名。</summary>
        public const string PositiveButtonElementName = "input-quantizer-basics-positive";

        /// <summary>負入力Button要素名。</summary>
        public const string NegativeButtonElementName = "input-quantizer-basics-negative";

        /// <summary>clamp確認Button要素名。</summary>
        public const string ClampButtonElementName = "input-quantizer-basics-clamp";

        /// <summary>非有限値確認Button要素名。</summary>
        public const string RejectNonFiniteButtonElementName = "input-quantizer-basics-reject-non-finite";
        ...
        private AxisQuantizer _quantizer;
        private short _currentValue;
        private double _lastInput;
        private InputQuantizationError _lastError;
        private bool _nonFiniteRejected;
        private int _buttonActionCount;

        /// <summary>最後に成功した量子化値。</summary>
        public short CurrentValue => _currentValue;

        /// <summary>最後に操作した入力。NaNもそのまま保持する。</summary>
        public double LastInput => _lastInput;

        /// <summary>最後の量子化error。</summary>
        public InputQuantizationError LastError => _lastError;
        ...
        private void BuildUi()
        {
            ...
            _title = AddLabel(TitleElementName, "Input Quantizer Basics", 31f, new Color(0.96f, 0.96f, 1f, 1f));
            _title.style.unityFontStyleAndWeight = FontStyle.Bold;
            _description = AddLabel(DescriptionElementName, "連続入力をdead zone付きの小さな整数commandへ決定論的に変換します。", 15f, new Color(0.82f, 0.84f, 0.98f, 1f));
            _configuration = AddLabel(ConfigurationElementName, "DEAD ZONE ±0.10  ·  8 STEPS / SIDE  ·  HALF AWAY  ·  CLAMP", 12f, new Color(0.55f, 1f, 0.82f, 1f));
            ...
            _buttons = new[]
            {
                CreateButton(DeadZoneButtonElementName, "Dead Zone +0.05", () => Apply(0.05d, "Inside dead zone  ·  inclusive zero")),
                CreateButton(PositiveButtonElementName, "Quantize +0.55", () => Apply(0.55d, "Positive input  ·  command +4")),
                CreateButton(NegativeButtonElementName, "Quantize -0.55", () => Apply(-0.55d, "Negative input  ·  command -4")),
                CreateButton(ClampButtonElementName, "Clamp +2", () => Apply(2d, "Out of range  ·  clamped command +8")),
        ...
        private void Apply(double input, string stage)
        {
            _lastInput = input;
            var quantized = _quantizer.Quantize(input);
            _lastError = quantized.Error;
            if (quantized.Succeeded) _currentValue = quantized.Value;
        ...
        {
            var before = _currentValue;
            _lastInput = double.NaN;
            var quantized = _quantizer.Quantize(_lastInput);
            _lastError = quantized.Error;
            _nonFiniteRejected = !quantized.Succeeded && quantized.Error == InputQuantizationError.NonFiniteInput && _currentValue == before;
            _buttonActionCount++;
        ...
        {
            if (!AxisQuantizer.TryCreate(0.1d, 8, out _quantizer, out _lastError)) throw new InvalidOperationException("Input Quantizer Basics configuration is invalid.");
            _currentValue = 0;
            _lastInput = 0d;
            _nonFiniteRejected = false;
        ...
        {
            var inputText = double.IsNaN(_lastInput) ? "NaN" : _lastInput.ToString("+0.00;-0.00;0.00", System.Globalization.CultureInfo.InvariantCulture);
            _input.text = $"INPUT {inputText}   ·   COMMAND {_currentValue}   ·   ACTIONS {_buttonActionCount}";
            _result.text = $"ERROR {_lastError}   ·   NON-FINITE PRESERVED {_nonFiniteRejected}";
        }
