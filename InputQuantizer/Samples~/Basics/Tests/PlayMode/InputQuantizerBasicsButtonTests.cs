...
namespace InputQuantization.Samples.PlayMode.Tests
{
    /// <summary>import済みBasicsの実Button、量子化結果、responsive geometryを検証する。</summary>
    [Parallelizable(ParallelScope.None)]
    [UnityPlatform(RuntimePlatform.WindowsEditor, RuntimePlatform.OSXEditor, RuntimePlatform.LinuxEditor)]
    public sealed class InputQuantizerBasicsButtonTests
    {
        private const string PanelSettingsGuid = "b7000000000000000000000000000010";
        private GameObject _host;
        private UIDocument _document;
        private InputQuantizerBasicsController _sample;
        ...
        public IEnumerator SetUp()
        {
            ...
            _host = new GameObject("Input Quantizer Basics Test Host");
            _host.SetActive(false);
            _document = _host.AddComponent<UIDocument>();
            _document.panelSettings = _panelSettings;
            _sample = _host.AddComponent<InputQuantizerBasicsController>();
            _host.SetActive(true);
            yield return WaitUntil(() => ReadyRoot()?.Q<VisualElement>(InputQuantizerBasicsController.CardElementName)?.worldBound.width > 0f, "960x600の実panelとsampleが準備されませんでした。");
            yield return null;
        }
        ...
        {
            Assert.That(_sample.CurrentValue, Is.Zero);
            Assert.That(_sample.LastInput, Is.Zero);
            Assert.That(_sample.LastError, Is.EqualTo(InputQuantizationError.None));
            Assert.That(_sample.NonFiniteRejected, Is.False);
            Assert.That(_sample.ButtonActionCount, Is.Zero);
            yield return null;
        }

        /// <summary>実Button列が0、+4、-4、clamp+8のgolden sequenceへ到達する。</summary>
        [UnityTest]
        public IEnumerator GoldenButtons_ProduceSymmetricClampedCommands()
        {
            Click(InputQuantizerBasicsController.DeadZoneButtonElementName);
            Assert.That(_sample.CurrentValue, Is.Zero);
            Click(InputQuantizerBasicsController.PositiveButtonElementName);
            Assert.That(_sample.CurrentValue, Is.EqualTo(4));
            Click(InputQuantizerBasicsController.NegativeButtonElementName);
            Assert.That(_sample.CurrentValue, Is.EqualTo(-4));
            Click(InputQuantizerBasicsController.ClampButtonElementName);
            Assert.That(_sample.CurrentValue, Is.EqualTo(8));
            Assert.That(_sample.LastInput, Is.EqualTo(2d));
            Assert.That(_sample.LastError, Is.EqualTo(InputQuantizationError.None));
        ...
        public IEnumerator NonFiniteButton_PreservesLastSuccessfulValue()
        {
            ...
            Click(InputQuantizerBasicsController.RejectNonFiniteButtonElementName);
            Assert.That(double.IsNaN(_sample.LastInput), Is.True);
            Assert.That(_sample.CurrentValue, Is.EqualTo(before));
            Assert.That(_sample.LastError, Is.EqualTo(InputQuantizationError.NonFiniteInput));
        ...
        {
            var root = ReadyRoot();
            var card = root.Q<VisualElement>(InputQuantizerBasicsController.CardElementName);
            var names = new[]
            {
                InputQuantizerBasicsController.TitleElementName,
                InputQuantizerBasicsController.DescriptionElementName,
                InputQuantizerBasicsController.ConfigurationElementName,
                InputQuantizerBasicsController.InputElementName,
                InputQuantizerBasicsController.StageElementName,
                InputQuantizerBasicsController.ResultElementName,
                InputQuantizerBasicsController.DeadZoneButtonElementName,
                InputQuantizerBasicsController.PositiveButtonElementName,
                InputQuantizerBasicsController.NegativeButtonElementName,
                InputQuantizerBasicsController.ClampButtonElementName,
                InputQuantizerBasicsController.RejectNonFiniteButtonElementName
            };
            ...
            var buttons = new[]
            {
                root.Q<Button>(InputQuantizerBasicsController.DeadZoneButtonElementName),
                root.Q<Button>(InputQuantizerBasicsController.PositiveButtonElementName),
                root.Q<Button>(InputQuantizerBasicsController.NegativeButtonElementName),
                root.Q<Button>(InputQuantizerBasicsController.ClampButtonElementName),
                root.Q<Button>(InputQuantizerBasicsController.RejectNonFiniteButtonElementName)
            };
            if (wide)
            {
        ...
        {
            var target = new RenderTexture(width, height, 0, RenderTextureFormat.ARGB32) { name = $"Input Quantizer Test {width}x{height}" };
            target.Create();
            return target;
        }
