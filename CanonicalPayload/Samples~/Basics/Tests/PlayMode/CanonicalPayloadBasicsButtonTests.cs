...
namespace CanonicalPayload.Samples.PlayMode.Tests
{
    /// <summary>import済みBasicsの実Button、canonical payload、responsive geometryを検証する。</summary>
    [Parallelizable(ParallelScope.None)]
    [UnityPlatform(RuntimePlatform.WindowsEditor, RuntimePlatform.OSXEditor, RuntimePlatform.LinuxEditor)]
    public sealed class CanonicalPayloadBasicsButtonTests
    {
        private const string PanelSettingsGuid = "e4000000000000000000000000000012";
        private GameObject _host;
        private UIDocument _document;
        private CanonicalPayloadBasicsController _sample;
        ...
        public IEnumerator SetUp()
        {
            ...
            _host = new GameObject("Canonical Payload Basics Test Host");
            _host.SetActive(false);
            _document = _host.AddComponent<UIDocument>();
            _document.panelSettings = _panelSettings;
            _sample = _host.AddComponent<CanonicalPayloadBasicsController>();
            _host.SetActive(true);
            yield return WaitUntil(() => ReadyRoot()?.Q<VisualElement>(CanonicalPayloadBasicsController.CardElementName)?.worldBound.width > 0f, "960x600の実panelとsampleが準備されませんでした。");
            yield return null;
        }
        ...
        /// <summary>初期状態が有効な空payloadで安定する。</summary>
        [UnityTest]
        public IEnumerator InitialState_IsEmptyStablePayload()
        {
            Assert.That(_sample.LastPayload.IsValid, Is.True);
            Assert.That(_sample.PayloadByteCount, Is.Zero);
            Assert.That(_sample.PayloadHex, Is.Empty);
            Assert.That(_sample.ButtonActionCount, Is.Zero);
            Assert.That(_sample.DecodeVerified, Is.False);
            Assert.That(_sample.CorruptionRejected, Is.False);
            Assert.That(_sample.RebuildMatched, Is.False);
            yield return null;
        }

        /// <summary>Encode、Decode、破損拒否、再Buildが同じcanonical bytesを保つ。</summary>
        [UnityTest]
        public IEnumerator EncodeDecodeCorruptRebuild_PreservesGoldenPayload()
        {
            Click(CanonicalPayloadBasicsController.EncodeButtonElementName);
            Assert.That(_sample.PayloadHex, Is.EqualTo(CanonicalPayloadBasicsController.ExpectedPayloadHex));
            Assert.That(_sample.PayloadByteCount, Is.EqualTo(30));
            var bytes = _sample.LastPayload.ToByteArray();
            Click(CanonicalPayloadBasicsController.DecodeButtonElementName);
            Assert.That(_sample.DecodeVerified, Is.True);
            Click(CanonicalPayloadBasicsController.CorruptButtonElementName);
            Assert.That(_sample.CorruptionRejected, Is.True);
            Assert.That(_sample.LastPayload.ToByteArray(), Is.EqualTo(bytes));
            Click(CanonicalPayloadBasicsController.RebuildButtonElementName);
            Assert.That(_sample.RebuildMatched, Is.True);
            Assert.That(_sample.PayloadHex, Is.EqualTo(CanonicalPayloadBasicsController.ExpectedPayloadHex));
            Assert.That(_sample.ButtonActionCount, Is.EqualTo(4));
            yield return null;
        }

        /// <summary>Resetがpayloadと全結果を初期状態へ戻す。</summary>
        [UnityTest]
        public IEnumerator ResetButton_RestoresInitialState()
        {
            ...
            Click(CanonicalPayloadBasicsController.CorruptButtonElementName);
            Click(CanonicalPayloadBasicsController.RebuildButtonElementName);
            Click(CanonicalPayloadBasicsController.ResetButtonElementName);
            Assert.That(_sample.LastPayload.IsValid, Is.True);
            Assert.That(_sample.PayloadByteCount, Is.Zero);
            Assert.That(_sample.PayloadHex, Is.Empty);
            Assert.That(_sample.ButtonActionCount, Is.Zero);
        ...
        {
            var root = ReadyRoot();
            var card = root.Q<VisualElement>(CanonicalPayloadBasicsController.CardElementName);
            var names = new[]
            {
                CanonicalPayloadBasicsController.TitleElementName,
                CanonicalPayloadBasicsController.DescriptionElementName,
                CanonicalPayloadBasicsController.ConfigurationElementName,
                CanonicalPayloadBasicsController.PayloadElementName,
                CanonicalPayloadBasicsController.StageElementName,
                CanonicalPayloadBasicsController.ResultElementName,
                CanonicalPayloadBasicsController.EncodeButtonElementName,
                CanonicalPayloadBasicsController.DecodeButtonElementName,
                CanonicalPayloadBasicsController.CorruptButtonElementName,
                CanonicalPayloadBasicsController.RebuildButtonElementName,
                CanonicalPayloadBasicsController.ResetButtonElementName
            };
            ...
            var buttons = new[]
            {
                root.Q<Button>(CanonicalPayloadBasicsController.EncodeButtonElementName),
                root.Q<Button>(CanonicalPayloadBasicsController.DecodeButtonElementName),
                root.Q<Button>(CanonicalPayloadBasicsController.CorruptButtonElementName),
                root.Q<Button>(CanonicalPayloadBasicsController.RebuildButtonElementName),
                root.Q<Button>(CanonicalPayloadBasicsController.ResetButtonElementName)
            };
            if (wide)
            {
        ...
        {
            var target = new RenderTexture(width, height, 0, RenderTextureFormat.ARGB32) { name = $"Canonical Payload Test {width}x{height}" };
            target.Create();
            return target;
        }
