...
namespace InputSequencing.Samples.PlayMode.Tests
{
    /// <summary>import済みBasicsの実Button、sequence契約、responsive geometryを検証する。</summary>
    [Parallelizable(ParallelScope.None)]
    [UnityPlatform(RuntimePlatform.WindowsEditor, RuntimePlatform.OSXEditor, RuntimePlatform.LinuxEditor)]
    public sealed class InputSequenceMatcherBasicsButtonTests
    {
        private const string PanelSettingsGuid = "ea000000000000000000000000000010";
        private GameObject _host;
        private UIDocument _document;
        private InputSequenceMatcherBasicsController _sample;
        ...
        public IEnumerator SetUp()
        {
            ...
            _host = new GameObject("Input Sequence Matcher Basics Test Host");
            _host.SetActive(false);
            _document = _host.AddComponent<UIDocument>();
            _document.panelSettings = _panelSettings;
            _sample = _host.AddComponent<InputSequenceMatcherBasicsController>();
            _host.SetActive(true);
            yield return WaitUntil(() => ReadyRoot()?.Q<VisualElement>(InputSequenceMatcherBasicsController.CardElementName)?.worldBound.width > 0f, "960x600の実panelとsampleが準備されませんでした。");
            yield return null;
        }
        ...
        /// <summary>初期tick、空進捗、次の期待command、操作数が設定どおりで安定する。</summary>
        [UnityTest]
        public IEnumerator InitialState_IsReadyAtTick100()
        {
            Assert.That(_sample.CurrentTick, Is.EqualTo(100));
            Assert.That(_sample.Progress, Is.Zero);
            Assert.That(_sample.ExpectedCommandId, Is.EqualTo(1));
            Assert.That(_sample.LastError, Is.EqualTo(InputSequenceError.None));
            Assert.That(_sample.ButtonActionCount, Is.Zero);
            yield return null;
        }

        /// <summary>Light・Light・Heavyが最大間隔内で一致して進捗0へ戻る。</summary>
        [UnityTest]
        public IEnumerator GoldenButtons_LightLightHeavyMatches()
        {
            Click(InputSequenceMatcherBasicsController.FirstLightButtonElementName);
            Assert.That(_sample.CurrentTick, Is.EqualTo(100));
            Assert.That(_sample.Progress, Is.EqualTo(1));
            Assert.That(_sample.LastMatched, Is.False);
            Click(InputSequenceMatcherBasicsController.SecondLightButtonElementName);
            Assert.That(_sample.CurrentTick, Is.EqualTo(101));
            Assert.That(_sample.Progress, Is.EqualTo(2));
            Click(InputSequenceMatcherBasicsController.HeavyButtonElementName);
            Assert.That(_sample.CurrentTick, Is.EqualTo(102));
            Assert.That(_sample.Progress, Is.Zero);
            Assert.That(_sample.LastMatched, Is.True);
            Assert.That(_sample.ButtonActionCount, Is.EqualTo(3));
            yield return null;
        }

        /// <summary>前回一致から最大間隔を超えたLightがtimeout後のpattern先頭として再開する。</summary>
        [UnityTest]
        public IEnumerator LateLight_TimesOutAndRestartsPattern()
        {
            Click(InputSequenceMatcherBasicsController.FirstLightButtonElementName);
            Click(InputSequenceMatcherBasicsController.SecondLightButtonElementName);
            Click(InputSequenceMatcherBasicsController.HeavyButtonElementName);
            Click(InputSequenceMatcherBasicsController.RestartLightButtonElementName);
            Assert.That(_sample.Progress, Is.EqualTo(1));
            Click(InputSequenceMatcherBasicsController.LateLightButtonElementName);
            Assert.That(_sample.CurrentTick, Is.EqualTo(106));
            Assert.That(_sample.Progress, Is.EqualTo(1));
            Assert.That(_sample.LastTimedOut, Is.True);
            Assert.That(_sample.LastMatched, Is.False);
        ...
        {
            var root = ReadyRoot();
            var card = root.Q<VisualElement>(InputSequenceMatcherBasicsController.CardElementName);
            var names = new[]
            {
                InputSequenceMatcherBasicsController.TitleElementName,
                InputSequenceMatcherBasicsController.DescriptionElementName,
                InputSequenceMatcherBasicsController.ConfigurationElementName,
                InputSequenceMatcherBasicsController.InputElementName,
                InputSequenceMatcherBasicsController.StageElementName,
                InputSequenceMatcherBasicsController.ResultElementName,
                InputSequenceMatcherBasicsController.FirstLightButtonElementName,
                InputSequenceMatcherBasicsController.SecondLightButtonElementName,
                InputSequenceMatcherBasicsController.HeavyButtonElementName,
                InputSequenceMatcherBasicsController.RestartLightButtonElementName,
                InputSequenceMatcherBasicsController.LateLightButtonElementName
            };
            ...
            var buttons = new[]
            {
                root.Q<Button>(InputSequenceMatcherBasicsController.FirstLightButtonElementName),
                root.Q<Button>(InputSequenceMatcherBasicsController.SecondLightButtonElementName),
                root.Q<Button>(InputSequenceMatcherBasicsController.HeavyButtonElementName),
                root.Q<Button>(InputSequenceMatcherBasicsController.RestartLightButtonElementName),
                root.Q<Button>(InputSequenceMatcherBasicsController.LateLightButtonElementName)
            };
            if (wide)
            {
        ...
        {
            var target = new RenderTexture(width, height, 0, RenderTextureFormat.ARGB32) { name = $"Input Sequence Matcher Test {width}x{height}" };
            target.Create();
            return target;
        }
