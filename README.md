...
| 新しいProjectごとに基本フォルダー、asmdef、設定、C#生成設定、複製名、開始Sceneを手作業したくない | [プロジェクト一括設定（Project Setup）](ProjectSetup/) | `Assets`配下の基本フォルダーとRuntime・Editor用asmdef、Project Settings、Root Namespace、新規scriptの改行方式、複製時の命名規則、Play Mode開始Scene、条件付きコンパイル記号、Tag・Layer、Build Scenesをbackup付きでまとめて適用する。 |
