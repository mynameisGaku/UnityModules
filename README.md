...
| 新しいProjectごとに同じ設定、C#生成設定、開始Sceneを手作業したくない | [プロジェクト一括設定（Project Setup）](ProjectSetup/) | Project Settings、Root Namespace、新規scriptの改行方式、Play Mode開始Scene、条件付きコンパイル記号、Tag・Layer、Build Scenesをbackup付きでまとめて適用する。 |
...
| [プロジェクト一括設定（Project Setup）](ProjectSetup/) | Project Settings、Root Namespace、新規scriptの改行方式、Play Mode Start Scene、条件付きコンパイル記号、Tag・Layer・Sorting Layer、Build Scenesをprofile化する。差分Preview、backup、適用、復元を一つのEditor画面で行う。**Unity 6000.5 以降**。 | com.unity.modules.uielements 1.0.0 |
