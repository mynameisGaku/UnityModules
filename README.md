...
| 再生中に調整したインスペクターの値を終了後も残すため、控えて入力し直したくない | [実行中調整](PlayModeTuning/) | 残したいコンポーネントの項目を先に選び、再生中に手動で記録し、終了後の差分と内容を確認してからシーンへ反映する。 |
...
| [実行中調整](PlayModeTuning/) | 保存済みシーンの`MonoBehaviour`から残したい最上位の`SerializedProperty`を選び、再生中に手動で記録し、終了後の差分表示、古くなった計画の拒否、反映後確認、失敗時の復元結果、取り消し履歴を一つの画面で扱うエディター専用モジュール。**Unity 6000.5.7f1以降**。 | なし |
...
再生中の調整値を残す場合は、[実行中調整](PlayModeTuning/)を個別に追加して`Tools > 実行中調整 > 開く`を開く。①「対象を選ぶ」、②「再生中の値を記録する」、③「再生終了後に差分を見る」、④「変更内容を確認する」、⑤「変更を反映して結果を見る」の順に進み、選択項目だけを手動で記録してから差分を反映する。
...
└── Modules/
    ...
    │   ├── Tests/Editor/    PlayModeTuning.Editor.Tests
    │   └── Documentation~/  操作順と実画面
    ├── Inspector/
