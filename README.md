...
| 新しいProjectごとに同じ設定、Play Mode開始Scene、Build Scenesを手作業したくない | [プロジェクト一括設定（Project Setup）](ProjectSetup/) | Project Settings、Play Mode開始Scene、Tag・Layer、Build Scenesを差分確認してからbackup付きでまとめて適用する。 |
...
| [プロジェクト一括設定（Project Setup）](ProjectSetup/) | Project Settings、Play Mode Start Scene、Tag・Layer・Sorting Layer、Build Scenesの順序とEnabled状態をprofile化する。差分Preview、backup、適用、直前backupへの復元を一つのEditor画面で行う。**Unity 6000.5 以降**。 | com.unity.modules.uielements 1.0.0 |
...
新規Projectの設定をそろえる場合は、[プロジェクト一括設定（Project Setup）](ProjectSetup/) を追加して `Tools > Project Setup > Open` を開く。`New recommended profile` で安全な推奨profileを作り、必要ならPlay Mode Start Scene、Tag・Layer・Sorting Layer、Build Scenesを追加する。`Preview changes` で差分を確認してから `Apply profile` を実行すると、適用直前の設定が自動backupされ、同じ画面から復元できる。
