...
| [プロジェクト初期設定（Project Setup）](ProjectSetup/) | Asset Serialization、Version Control、Enter Play Mode、Color Space、Run In Background、Company Name、Product Name、Bundle Versionをprofile化し、差分Preview、backup、適用、直前backupへの復元を一つのEditor画面で行う。**Unity 6000.5 以降**。 | com.unity.modules.uielements 1.0.0 |
...
新規Projectの設定をそろえる場合は、[プロジェクト初期設定（Project Setup）](ProjectSetup/) を追加して `Tools > Project Setup > Open` を開く。`New recommended profile` で安全な推奨profileを作り、`Preview changes` で差分を確認してから `Apply profile` を実行する。適用直前の設定は自動backupされ、同じ画面から復元できる。
